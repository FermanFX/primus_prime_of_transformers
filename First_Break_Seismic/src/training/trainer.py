...
class SeismicTrainer:
    ...
    def __init__(
        ...
        self.registered_models: dict[str, Any] = {}  # Track registered models for alias management
    ...
    def train_epoch(self, verbose: bool = False) -> tuple[float, dict]:
        ...
        for batch_idx, (x, y) in enumerate(pbar):
            ...
            if has_components:
                ...
                components = cast(dict[str, Any], components_raw)
                
                # Accumulate component losses
                loss_components['total'] += float(components['total'])
                loss_components['ce'] += float(components['ce'])
                loss_components['focal'] += float(components['focal'])
                loss_components['dice'] += float(components['dice'])
                loss_components['ce_focal_combined'] += float(components['ce_focal_combined'])
                
                per_class = cast(dict[str, float], components['per_class'])
                for class_name, class_loss in per_class.items():
                    ...
                    loss_components['per_class'][class_name] += float(class_loss)
            else:
        ...
        if num_batches > 0 and getattr(self.criterion, "return_components", False):
            for key in ['total', 'ce', 'focal', 'dice', 'ce_focal_combined']:
                metrics[f'loss_{key}'] = loss_components[key] / num_batches
        
            per_class_total = cast(dict[str, float], loss_components['per_class'])
            for class_name, class_loss in per_class_total.items():
