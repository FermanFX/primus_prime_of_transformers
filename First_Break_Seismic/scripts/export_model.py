...
@click.option("--onnx", "onnx_flag", is_flag=True, help="Export to ONNX format")
@click.option(
    "--torchscript",
    "torchscript_flag",
    is_flag=True,
...
def main(
    ...
    if not onnx_flag and not torchscript_flag:
        print(
    ...
    if onnx_flag:
        ...
        try:
            ...
            torch.onnx.export(
                model_obj,
                (example_input,),  # Wrapped in a tuple as expected by mypy/torch
                onnx_path,
                input_names=["input"],
                output_names=["output"],
