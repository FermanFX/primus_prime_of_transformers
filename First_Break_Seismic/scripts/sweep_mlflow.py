...
class SweepExperiment:
    ...
    def run_experiment(
        ...
        try:
            ...
            if self.mlflow_manager:
                run_id = self.mlflow_manager.start_run(
                    ...
                    tags={
                        ...
                        "sweep_id": datetime.now(UTC).strftime("%Y%m%d_%H%M%S"),
            ...
            logger.error(
                f"   ❌ TIMEOUT after {self.global_config.get('timeout_seconds', 3600)}s"
            )
            return {
                "success": False,
                "dataset": dataset,
    ...
    def parse_metrics(output: str) -> dict:
        ...
        Extracts ``Train Loss``, ``Val Loss``, ``Train IoU``, ``Val IoU``,
        ``Train Acc``, and ``Val Acc`` values from the training log. Handles
        lines where values are followed by commas or other punctuation (e.g.
            ...
            for key, marker in patterns.items():
                if marker in line:
                    try:
                        ...
                        # Strip trailing punctuation that commonly appears
                        # after numeric values in log lines.
                        cleaned = raw.strip(",;")
                        metrics[key] = float(cleaned)
                    except (IndexError, ValueError) as e:
                        logger.debug(
                            f"Could not parse {key} from: {line[:50]}... Error: {e}"
                        )
