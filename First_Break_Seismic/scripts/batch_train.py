...
def run_auto_batch_training(
    ...
    cpu_rec = recommendations.get("cpu")

    if info["pytorch"]["cuda_available"] and cuda_rec is not None:
        device_type = "cuda"
        available_gb = cuda_rec["recommended_gb"]
        device_name = info["cuda"]["devices"][0]["name"]
        device_memory_gb = info["cuda"]["devices"][0]["total_gb"]
    elif info["pytorch"]["mps_available"] and mps_rec is not None:
        device_type = "mps"
        available_gb = mps_rec["recommended_gb"]
        device_name = "Apple Silicon (MPS)"
        device_memory_gb = info["mps"]["system_ram_gb"]
    elif cpu_rec is not None:
        device_type = "cpu"
        available_gb = cpu_rec["recommended_gb"]
        device_name = "CPU"
    ...
    for dataset_name in selected_datasets:
        ...
        for model_name in model_order:
            ...
            if config:
                ...
                unique_variants = []
                for v in variants:
                    key_tuple = (v["batch_size"], v["cache_size"], v["memory_limit_gb"])
