...
def create_synthetic_hdf5(filepath: Path, num_shots: int = 5, total_traces: int = 100, num_samples: int = 751):
    ...
    with h5py.File(filepath, "w") as f:
        ...
        group.create_dataset("data_array", data=np.random.randn(actual_total, num_samples).astype(np.float32))
        group.create_dataset("SPARE1", data=np.random.randint(50, 150, size=(actual_total, 1)).astype(np.float32))
...
@pytest.fixture
def sample_config(tmp_path, synthetic_hdf5):
...
def test_pipeline_end_to_end(sample_config):
...
def test_force_reprocess_flag(sample_config):
...
def test_invalid_hdf5_error_handling(tmp_path):
